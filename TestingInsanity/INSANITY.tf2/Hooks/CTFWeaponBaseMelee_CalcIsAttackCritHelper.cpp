...
MAKE_HOOK(CTFWeaponBaseMelee_CalcIsAttackCriticalHelper, "48 89 74 24 ? 57 48 83 EC ? 48 8B F9 E8 ? ? ? ? 48 8B C8 C7 44 24 ? ? ? ? ? 4C 8D 0D ? ? ? ? 33 D2 4C 8D 05 ? ? ? ? E8 ? ? ? ? 48 8B F0 48 85 C0 0F 84 ? ? ? ? 48 8B 10", __fastcall, CLIENT_DLL,
    bool, baseWeapon* pActiveWeapon, void* pLocalPlayer)
{
    ...
    printf("--> called! <-- ( make sure it ain't been called for other players on the server, which it shouldn't (cause common sense :| ))\n");

    return Hook::CTFWeaponBaseMelee_CalcIsAttackCriticalHelper::O_CTFWeaponBaseMelee_CalcIsAttackCriticalHelper(pActiveWeapon, pLocalPlayer);
}