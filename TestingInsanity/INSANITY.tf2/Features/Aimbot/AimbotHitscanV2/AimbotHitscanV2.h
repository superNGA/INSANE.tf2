...
public:
    ...
    BaseEntity* GetTargetEntity() const;

private:
    // Auto-fire & other convinence features...
    ...
    BaseEntity* _ChooseBuildingTarget  (const std::vector<BaseEntity*>* pVecTargets, BaseEntity* pLocalPlayer, baseWeapon* pActiveWeapon, CUserCmd* pCmd);
    BaseEntity* _ChooseStickyBombTarget(const std::vector<BaseEntity*>* pVecTargets, BaseEntity* pLocalPlayer, baseWeapon* pActiveWeapon, CUserCmd* pCmd);
    ...
    BaseEntity* m_pLastTarget = nullptr;
...
DEFINE_FEATURE(AimbotHitscan_Multipoint,      "Multi Hitpoints",         IntSlider_t,   AimbotHitscanV2, Aimbot, 11, IntSlider_t(0, 0, 8), FeatureFlag_None, "Perform visibility check on multiple points on single hitbox. ( Shoots faster )")
// NOTE : These bloom radiuses range from 0.1 to 1.0, and the will be halved later to represent range upto 0.5f. i.e. center to on edge.
DEFINE_FEATURE(AimbotHitscan_PlayerBloom,     "Visibility Bloom Player", FloatSlider_t, AimbotHitscanV2, Aimbot, 12, FloatSlider_t(0.3f, 0.1f, 1.0f), FeatureFlag_None, "How far away do you want the multi-Hitpoints. ( for players )")
DEFINE_FEATURE(AimbotHitscan_BuildingBloom,   "Visibility Bloom Builds", FloatSlider_t, AimbotHitscanV2, Aimbot, 13, FloatSlider_t(0.3f, 0.1f, 1.0f), FeatureFlag_None, "How far away do you want the multi-Hitpoints. ( for buildings )")
...
DEFINE_FEATURE(Hitbox_StickyBombs, "Sticky Bombs", bool, AimbotHitscan_Hitbox, Aimbot, 8, false)