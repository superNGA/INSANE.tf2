...
public:
	...
	virtual bool		RegisterModelLoadCallback(model_t* pModel, bool bClientOnly, void* pCallback, bool bCallImmediatelyIfLoaded = true) = 0;

	// Called by client code or IModelLoadCallback destructor
	virtual void		UnregisterModelLoadCallback(model_t* pModel, bool bClientOnly, void* pCallback) = 0;

	virtual void		Client_OnServerModelStateChanged(model_t* pModel, bool bServerLoaded) = 0;
};


MAKE_INTERFACE_SIGNATURE(iModelLoader, "48 8D 0D ? ? ? ? E8 ? ? ? ? 48 8B D8 8B 40", IModelLoader, ENGINE_DLL, 3, 7)