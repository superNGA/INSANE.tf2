...
void execute_thread2(HINSTANCE instance)
{
	...
	while (!directX::UI::UI_has_been_shutdown)
	{
		...
		const view_matrix r_viewmatrix	= entities::matrix_world_to_screen.load();
		...
		float last_best_distance		= 10000.0f; // this is just some obsurdly large value, not the FOV
		float distance_from_crosshair; 

		/* storing game time */
		static float last_game_time = 0.0f;
		static int last_tick = 0;

		/* Projectile aimbot vars */
		static bool projectile_aimbot_primed = false;
		vec2 projectile_AB_target_screen_pos;
		vec2 AB_target_chest_screen_pos;
		...
		for (int ent_num = 0; ent_num < ent_count; ent_num++)
		{
			...
			if (CHE_playerInfo.name[0] == '\0') continue;
			...
			/* getting BONES */
			if (!ent->SetupBones(CHE_mBones, MAX_STUDIO_BONES, HITBOX_BONES, p_globalvar->curtime)) continue; // skipping loop if setup bones fail?
			...
			{
				vec ent_vel;
				if (last_tick != p_globalvar->tickcount && config::aimbot::projectile_aimbot)
				{
					vec old_ent_pos = map_entity_pos[std::string(CHE_playerInfo.name)];
					...
					/* updaing time for software */
					last_tick = p_globalvar->tickcount;
					last_game_time = p_globalvar->curtime;

					/* updating stored position */
					map_entity_pos[std::string(CHE_playerInfo.name)] = ent_pos;
					projectile_aimbot_primed = true;
				}
				
				/* deciding between PROJECTILE aimbot or NORMAL aimbot */
				vec aimbot_entity_pos;
				config::aimbot::projectile_aimbot&& projectile_aimbot_primed ?
					aimbot_entity_pos = proj_aimbot_calc(CHE_mBones[BONE_CHEST].get_bone_coordinates(), ent_vel, (*(int32_t*)((uintptr_t)ent + netvar.m_fFlags)) & 1, viewangles_localplayer) :
					aimbot_entity_pos = CHE_mBones[entities::target::target_bone].get_bone_coordinates();
				
				/* finding view angles for target */
				qangle target_angles = entities::world_to_viewangles(entities::local::eye_pos, aimbot_entity_pos);
				
				/* FOV mechanism */
				vec2 target_screen_pos;
				entities::world_to_screen(CHE_mBones[entities::target::target_bone].get_bone_coordinates(), target_screen_pos, &r_viewmatrix);
				...
				/* storing info. about closest enemy to the crosshair */
				if (distance_from_crosshair < last_best_distance)            
				{
					entities::target::best_angle = target_angles;
					last_best_distance = distance_from_crosshair;

					entities::world_to_screen(aimbot_entity_pos, projectile_AB_target_screen_pos, &r_viewmatrix);
					entities::world_to_screen(CHE_mBones[BONE_CHEST].get_bone_coordinates(), AB_target_chest_screen_pos, &r_viewmatrix);
				}
			}
			...
			if (config::visuals::ESP)
			{
				...
				ent_on_screen += entities::world_to_screen(CHE_mBones[BONE_HEAD].get_bone_coordinates(), cached_entity_dimension.head, &r_viewmatrix);
				ent_on_screen += entities::world_to_screen(CHE_mBones[BONE_LEFT_SHOULDER].get_bone_coordinates(), cached_entity_dimension.left_shoulder, &r_viewmatrix);
				ent_on_screen += entities::world_to_screen(CHE_mBones[BONE_RIGHT_SHOULDER].get_bone_coordinates(), cached_entity_dimension.right_shoulder, &r_viewmatrix);
				ent_on_screen += entities::world_to_screen(CHE_mBones[BONE_LEFT_FOOT].get_bone_coordinates(), cached_entity_dimension.left_foot, &r_viewmatrix);
				ent_on_screen += entities::world_to_screen(CHE_mBones[BONE_RIGHT_FOOT].get_bone_coordinates(), cached_entity_dimension.right_foot, &r_viewmatrix);
				if (ent_on_screen)
				{
					entities::target::active_buffer_index ?
		...
		entities::target::target_future_positon_data.store(projectile_AB_target_screen_pos);
		entities::target::target_chest_pos.store(AB_target_chest_screen_pos);
